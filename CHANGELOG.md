...
- `Ui\UiEntryCatalog`: the modules' UI entries, read from every registered
  ...
  the contracts in force (`contracts`), which installed theme implements each
  (`hosts`), and the entries the matcher accepted against them (`modules`),
  which is what a host mounts. `coolms:install` runs the same match before it
  writes anything and refuses a module whose range the implementing theme's
  version does not include, by name (the platform rule: hosts implement
  contracts, modules offer entries).
