...
class Configuration implements ConfigurationInterface
{
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $rootNode
            ...
            ->info('Platform secret-store backend. The dev default reads $_ENV; a HashiCorp Vault driver reads a vault; \'stored\' defers to the module that keeps secrets the application itself holds.')
            ->addDefaultsIfNotSet()
            ->children()
            ->enumNode('driver')->values(['env', 'stored', 'vault'])->defaultValue('env')->info("Active backend. 'env' reads environment variables; 'stored' uses the module that keeps stored secrets (it registers coolms.secret_store.stored); 'vault' is HashiCorp Vault KV v2.")->end()
            ...
            ->arrayNode('key_ring')
            ->info('The at-rest master key ring, shared by every sealed value: the key everything is sealed under, and the previous one during a rotation. Named `filesystem` until 2026-09-22, when the file it used to configure left this package.')
            ->addDefaultsIfNotSet()
            ->children()
